...
#pragma config(Sensor, in1,    bMove,          sensorTouch)
...
#pragma config(Sensor, in8,    lMove,          sensorTouch)
...
char getStateBall() {
	...
	if (bVexAutonomousMode) {
		if (!SensorValue[bMove]) //if ! (not) is required so the default set up allows auto-movement, and MUST be overidden by a jumper before other states can be accepted
			return BALL_MOVE;
		...
		//else if (0=1) //I do not have a method of starting this yet...
			//return BALL_TRCK;
		else //if (SensorValue[bStay]) //this is assumed at this point
			return BALL_STAY;
	} else {
...
char getStateLids() {
	//l[open, shut, move]
	if (bVexAutonomousMode) {
		if (!SensorValue[lMove]) //if ! (not) is required so the default set up allows auto-movement, and MUST be overidden by a jumper before other states can be accepted
			return LIDS_MOVE;
		else if (SensorValue[lShut])
			return LIDS_SHUT;
		else //if (SensorValue[lOpen]) //this is assumed at this point
			return LIDS_OPEN;
	} else {
		//Controller input, which is currently unplanned - but this is where I'd add it if I want to scare guests lol
		return LIDS_OPEN;
